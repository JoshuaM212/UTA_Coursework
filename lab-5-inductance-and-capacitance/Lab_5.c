...
void initHw()
{
    ...
    COMP_ACREFCTL_R |= COMP_ACREFCTL_EN | COMP_ACREFCTL_VREF_M;  // 0x020F   Ideal step size = VDDA/29.4  &   Ideal Vref = 2.469 V
    COMP_ACCTL1_R |= COMP_ACCTL1_ASRCP_REF |  COMP_ACCTL1_ISEN_BOTH; // 0x040C;
...
int main(void)
{
    ...
    while (true)
    {
        ...
        {
            Measure_Water_Level();
            snprintf(str, sizeof(str), "Time:  %7"PRIu32"\r\n", time);
            putsUart0(str);
            snprintf(str, sizeof(str), "Result from Water Level:  %7"PRIu32"\r\n\r\n", Comp_Result);
            putsUart0(str);
        }
        else
        {
            ...
            putsUart0(str);
            if (frequency >= 146000 && frequency <= 155000)
            {
                putsUart0("Coil Detected: ALUMINUM BOTTLE\r\n");
                BLUE_LED = 1;
            }
            else if (frequency >= 260000 && frequency <= 275000)
            {
                putsUart0("Coil Detected: LAPTOP\r\n");
                BLUE_LED = 1;
                GREEN_LED = 1;
            }
            else if (frequency >= 175000 && frequency <= 195000)
            {
                putsUart0("Coil Detected: POWER SUPPLY\r\n");
                GREEN_LED = 1;
            }
            else
            {
