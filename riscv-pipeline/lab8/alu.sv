    ...
    begin
        case(opcode)
            ...
            LOAD_opcode:  // Full list of I-types - JALR moved below 
                case(funct3)
                    funct3_0: alu_out = rs1_data_in; // $signed(RS1_Instr_comb[7:0]);  // LB
                    funct3_1: alu_out = rs1_data_in; // $signed(RS1_Instr_comb[15:0]); // LH
                    funct3_2: alu_out = rs1_data_in; // $signed(RS1_Instr_comb[31:0]); // LW
                    funct3_4: alu_out = rs1_data_in; // {24'b0, RS1_Instr_comb[7:0]};  // LBU
                    funct3_5: alu_out = rs1_data_in; // {16'b0, RS1_Instr_comb[15:0]}; // LHU
                    default:  alu_out = 32'b0; // Universal Default case
                endcase
            Immed_opcode:
                case(funct3)
