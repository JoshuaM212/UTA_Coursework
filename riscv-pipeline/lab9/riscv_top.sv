...
module riscv_top (
    ...
    wire        id_ex_str_we;
    wire        wb_from_mem_id_ex;
    wire [4:0] rs1_reg_id_ex;
    wire [4:0] rs2_reg_id_ex;
    ...
    wire [2:0]  mem_wb_wb_en_select;
    wire [1:0]  mem_wb_address_mod; 
    wire        wb_from_mem_mem_wb;
    ...
    // Data Hazard: Registered data from EX and MEM to ID
    wire df_wb_from_mem_ex;  
    wire df_wb_from_mem_mem;

    // Data Hazard: Registered data from MEM to WB to EX
    wire df_wb_from_wb_ex;

    // Stall Signal - Stops pc increment when load data hazard occurs
    wire stall;
	...
	.probe21(stall),            // input wire [0:0]  probe21 
    ...
    .stall(stall)                           // from id - stall signal new wire for lab 9
    );
    ...
    .wb_from_mem_out(wb_from_mem_id_ex),// to ex- new wire for lab 9 (should've been for lab 8)
    .rs1_reg_out(rs1_reg_id_ex),        // to ex - new wire for lab 9
    .rs2_reg_out(rs2_reg_id_ex),        // to ex - new wire for lab 9
    // Data Hazard Connections - from EX, MEM, and WB
    ...
    .stall(stall),                           // to id - stall signal new wire for lab 9
    .df_wb_from_mem_ex(df_wb_from_mem_ex),   // new wire for lab 9
    .df_wb_from_mem_mem(df_wb_from_mem_mem), // new wire for lab 9
    ...
    .wb_from_mem_in(wb_from_mem_id_ex),     // from id - new wire for lab 9 (should've been for lab 8)
    .rs1_reg_in(rs1_reg_id_ex),             // from id - new wire for lab 9 
    .rs2_reg_in(rs2_reg_id_ex),             // from id - new wire for lab 9 
    ...
    .store_we_out(ex_mem_str_we),           // to mem (from id) - new wire for lab 8
    .wb_from_mem_out(wb_from_mem_ex_mem),   // to mem- new wire for lab 9 (should've been for lab 8)
    
    .df_ex_enable(df_ex_enable),            // to id
    .df_ex_reg(df_ex_reg),                  // to id
    .df_ex_data(df_ex_data),                // to id
    .df_wb_from_mem_ex(df_wb_from_mem_ex),  // to id - new wire for lab 9

    .df_wb_from_mem_wb(df_wb_from_wb_ex),   // new wire for lab 9
    .df_wb_reg(df_wb_reg),                  // new wire for lab 9
    .df_wb_data(df_wb_data)                 // new wire for lab 9
    );
    ...
    .store_we_in(ex_mem_str_we),            // from ex - new wire for lab 8
    .rs2_data_in(ex_mem_rs1_data),          // from ex - new wire for lab 8
    .wb_from_mem_in(wb_from_mem_ex_mem),    // from ex - new wire for lab 9 (should've been for lab 8)
    ...
    .wb_from_mem_out(wb_from_mem_mem_wb),   // to wb - new wire for lab 9 (should've been for lab 8)

    // Memory Interface
    .memif_addr(mem_ram_addr),              // to dp ram   - new wire for lab 8
    .memif_rdata(mem_ram_rdata),            // from dp ram - new wire for lab 8
    ...
    .wb_from_mem_in(wb_from_mem_mem_wb),    // new wire for lab 9 (shouldve been from lab 8)
