/*
 * timer4.c
 *
 *  Created on: Apr 15, 2022
 *      Author: Dario Ugalde
 */

#include "nvic.h"
#include "tm4c123gh6pm.h"
#include "timer4.h"

void initTimer4()
{
    SYSCTL_RCGCTIMER_R |= SYSCTL_RCGCTIMER_R4;
    _delay_cycles(3);

    TIMER4_CTL_R &= ~TIMER_CTL_TAEN;                 // turn-off timer before reconfiguring
    TIMER4_CFG_R = TIMER_CFG_32_BIT_TIMER;           // configure as 32-bit timer (A+B)
    TIMER4_TAMR_R = TIMER_TAMR_TAMR_1_SHOT;          // configure for one-shot mode (count down)
    TIMER4_TAILR_R = 0x5;                            // set load value to 5 to interrupt every 125ns
    TIMER4_IMR_R = TIMER_IMR_TATOIM;                 // turn-on interrupts
    enableNvicInterrupt(INT_TIMER4A);                // turn-on interrupt 86 (TIMER4A)
}

void startTimer4()
{
    TIMER4_CTL_R |= TIMER_CTL_TAEN;
}

void stopTimer4()
{
    TIMER4_CTL_R &= ~TIMER_CTL_TAEN;
}

